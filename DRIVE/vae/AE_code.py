...
save_dir = 'models/my_model_ae/'
log_dir = 'models/logs_ae/'
sample_dir = 'models/samples_ae/'
...
def train(model, train_loader, optim, epoch):
    ...
    for input_image, output_image in tqdm(train_loader, desc="Train Set"):
        recon_batch = model(input_image)
        loss = loss_function(recon_batch[:, 0, :, :], (output_image * 255 / 12)[:, 0, :, :])
...
def test(model, test_loader):
    ...
    with torch.no_grad():
        for input_image, output_image in tqdm(test_loader, desc="Test Set"):
            recon_batch = model(input_image)
            test_loss += loss_function(recon_batch[:, 0, :, :], (output_image * 255 / 12)[:, 0, :, :]).item()
...
def main():
    ...
    model = AE(latent_size=LATENT_SPACE)
