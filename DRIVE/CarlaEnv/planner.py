    ...
    if plan is None:
        ...
        for i, action in enumerate(plan):
            ...
            while len(wp_choice) > 1:
                ...
                multi_path.append(wp_choice)
                wp0, wp1 = wp_choice[:2]
                if wp0.transform.location.distance(wp1.transform.location) < resolution:
                    step += resolution
                else:
                    break
            multi_path_T = list(map(list, zip(*multi_path)))
            ...
            if len(wp_choice) > 1:
                ...
                for multi_path_ in multi_path_T:
                    if current_waypoint in multi_path_:
                        for points in multi_path_:
                            route.append((points, action))
