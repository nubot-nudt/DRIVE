...
public:
    ...
    nubot_arm_node1()
    {
        ...
        /*********节点*********/
        arm_param_init();
        joy_sub_ = n.subscribe("/joy", 1, &nubot_arm_node1::joyCallback, this);
        controller = n.createTimer(ros::Duration(1.0 / 100.0), &nubot_arm_node1::timeController, this, false);
        // control_sub_ = n.subscribe("control_flag", 1, &nubot_arm_node1::linkCallback, this);
        // cobra_to_rviz_pub_ = n.advertise<nubot_msgs::cobraToRviz>("cobraToRviz", 1);
        firstCobraPubToInnfos_ = n.advertise<nubot_msgs::innfosSub>("innfosSub_left", 1, true);
        ...
        RHS_trans_ = n.advertise<std_msgs::Float64MultiArray>("/RHS_trans_l", 1, true);
        firstend_FT_ = n.advertise<std_msgs::Float64MultiArray>("/end_FT_l", 1, true);

        armL_jointState_pub = n.advertise<sensor_msgs::JointState>("/armL/joinState", 1, true);

        firstCobraSubFromInnfos_ = n.subscribe("innfosPub", 1, &nubot_arm_node1::firstCobraCallback, this);
        firstCobraPubToInnfosMsg.firstNeedMode = need_v;
    }

    ~nubot_arm_node1() // 关闭节点
    {
    ...
    void firstCobraCallback(const nubot_msgs::innfosPub &firstCobra)
    {
        ...
        for (int i = 0; i < N1; i++)
        {
            ...
            joint_vel[i] = DH_sign[i] * (M_PI * 2 / Motor_Reduction_Ratio[i]) * firstCobra.firstOriginalVel[i] / 60; // 除以减速比乘以2*M_PI/60转化成弧度/s
            // joint_vel[i] = firstCobra.firstOriginalVel[i];
            if (abs(joint_vel[i]) < 0.005)
                joint_vel[i] = 0;
        }
        for (int i = 0; i < 6; i++)
            firstend_FT.data[i] = firstCobra.firstEndFS[i];
    ...
    void timeController(const ros::TimerEvent &e)
    {
        ...
        else
        {
            ...
            n.getParam("firstLimitMax", LimitMax);
            n.getParam("firstLimitMin", LimitMin);

            /*************算出机械臂各关节信息并输出到屏幕********/
            cout << "\033[47;31m左臂\033[0m各电机角度，单位：度以及底层电机控制量速度，单位：rpm:" << endl;
            for (int i = 0; i < N1; i++)
            {
                ...
                // cout << "];";
                cout << "速度控制指令: " << setiosflags(ios::left) << setw(6) << firstCobraPubToInnfosMsg.firstSpeed[i] << ";";
            ...
            if (left_or_right == 2 || left_or_right == 1)
            {
                ...
                if (endOrJointMode == JOINT_MODE) // 一般控制
                {
                    ...
                    else if (flag_back) // 一键还原  还原过程中不能松开按键 否则将停止还原   还原完毕需要松开“↓”和→
                    {
                        ...
                        if (flag_need_set_ModeP) // 在if(flag_back)循环中，只执行一次这个子循环，即只设置一次位置模式，否则会出现：设置位置模式→控制电机位置→设置位置模式→控制电机位置→...一直循环这样会使电机不动。
                        {
                            ...
                            // firstCobraPubToInnfosMsg.firstNeedMode = need_p;
                            // firstCobraPubToInnfosMsg.firstPos[0] = pos[0] / (360.0 / Motor_Reduction_Ratio[0]);
                            // firstCobraPubToInnfosMsg.firstPos[1] = pos[1] / (360.0 / Motor_Reduction_Ratio[1]);
                            // firstCobraPubToInnfosMsg.firstPos[2] = pos_init[2] / (360.0 / Motor_Reduction_Ratio[2]);
                            // firstCobraPubToInnfosMsg.firstPos[3] = pos[3] / (360.0 / Motor_Reduction_Ratio[3]);
                            // firstCobraPubToInnfosMsg.firstPos[4] = pos[4] / (360.0 / Motor_Reduction_Ratio[4]);
                            // firstCobraPubToInnfosMsg.firstPos[5] = pos[5] / (360.0 / Motor_Reduction_Ratio[5]);
                            // firstCobraPubToInnfosMsg.firstPos[6] = pos[6] / (360.0 / Motor_Reduction_Ratio[6]);
                            // firstCobraPubToInnfosMsg.firstPosMode = 1;
                            // firstCobraPubToInnfos_.publish(firstCobraPubToInnfosMsg);
                    ...
                    for (int i = 0; i < N1; i++)
                    {
                        // firstCobraPubToInnfosMsg.firstSpeed[i] = ((1 + Is_ChangeV[i] * (cobra_Vel_level - 1)) * flag[i] * cobra_motor_sign[i]) * (2 * M_PI) / 180; // 仿真环境下需要修改单位为 弧度每秒
                        firstCobraPubToInnfosMsg.firstSpeed[i] = (1 + Is_ChangeV[i] * (cobra_Vel_level - 1)) * cobra_Vel[i] * flag[i] * cobra_motor_sign[i]; // 发布速度单位为....转每分钟
                    }
                ...
                else if (endOrJointMode == END_MODE) // 末端控制
                {
                    ...
                    for (int i = 0; i < N1; i++)
                        armLjointState.velocity[i] = joint_vel[i];
                    armL_jointState_pub.publish(armLjointState);
                    for (int i = 0; i < 6; i++)
                    {
                        firstend_vel_fb.data[i] = FbEndmove[i];
                    }
                    firstend_vel_d.data[0] = dx;
                    firstend_vel_d.data[1] = dy;
                    firstend_vel_d.data[2] = dz;
                    firstend_vel_d.data[3] = rx;
                    firstend_vel_d.data[4] = ry;
                    firstend_vel_d.data[5] = rz;
                    ...
                    for (int i = 0; i < N1; i++)
                        // firstCobraPubToInnfosMsg.firstSpeed[i] = DH_sign[i] * dtheta[i]; // 仿真环境下需要修改单位为弧度每秒
                        firstCobraPubToInnfosMsg.firstSpeed[i] = DH_sign[i] * dtheta[i] * Motor_Reduction_Ratio[i] * 60.0 / (3.1415926 * 2);
                    firstCobraPubToInnfos_.publish(firstCobraPubToInnfosMsg);
                }
            }
            firstend_FT_.publish(firstend_FT);
            if (endOrJointMode == JOINT_MODE)
    ...
    void
    arm_param_init() // 变量初始化 *(a.m_storage.m_data)@9 在调试中查看矩阵或者向量中的值
    {
        ...
        Motor_Reduction_Ratio.resize(N1);
        Motor_Reduction_Ratio << 36, 36, 36, 36, 36, 36, 36; // 各电机减速比
        temperature = VectorXd::Zero(N1);                    // 电机的温度
        DianLiu = VectorXd::Zero(N1);                        // 电机的电流

        firstSpeed = VectorXd::Zero(N1);   // 速度指令
        original_pos = VectorXd::Zero(N1); // 底层电机位置 单位：转
        pos = VectorXd::Zero(N1);          // 各电机位置信息 单位：°
        pos_init = VectorXd::Zero(N1);     // 初始化会自动录入启动时的初始位置
        pos_extend = VectorXd::Zero(N1);
        theta = VectorXd::Zero(N1);   // 末端控制状态电机角度。单位弧度。
        dtheta = VectorXd::Zero(N1);  // 末端控制状态解算的电机控制速度。单位弧度/s。
        end_move = VectorXd::Zero(6); // 末端运动的6个自由度

        pos_extend << 0, 0, 45, -90, 0, 0, 0; // 一键展开时的位置，重点在4号电机，方向不能反！！！！！
        Is_ChangeV.resize(N1);
        Is_ChangeV << 1, 1, 1, 1, 1, 1, 1; // 各电机是否受加减速影响，1为可以加减速，0为屏蔽加减速。
        DH_sign.resize(N1);
        DH_sign << -1, 1, -1, 1, -1, 1, -1; // 电机摆放对DH建模正方向的影响
        cobra_motor_sign.resize(N1);
        ...
        joint_vel = VectorXd::Zero(N1); // 各个电机反馈的转动速度 弧度/s

        firstend_pos_d.data.resize(6);
        firstend_pos_fb.data.resize(6);
        firstend_vel_d.data.resize(6);
        firstend_vel_fb.data.resize(6);
        firstend_FT.data.resize(6);
        RHS_trans.data.resize(9);
        armLjointState.name = {"jointL1", "jointL2", "jointL3", "jointL4", "jointL5", " jointL6 ", " jointL7 "};
        armLjointState.position = {0, 0, 0, 0, 0, 0, 0};
        armLjointState.velocity = {0, 0, 0, 0, 0, 0, 0};
        armLjointState.effort = {0, 0, 0, 0, 0, 0, 0};
    }
...
private:
    ...
    ros::Publisher firstend_FT_;
    ros::Publisher RHS_trans_;
    ros::Publisher armL_jointState_pub;
    ...
    std_msgs::Float64MultiArray firstend_FT;
    std_msgs::Float64MultiArray RHS_trans;
    sensor_msgs::JointState armLjointState;

    // 机械臂参数调节
    int N1 = 7;          // CAN上电机个数，可设置为常量值const
    ...
    std::vector<int> LimitMax = {100, 5, 160, 5, 100, 110, 105};         // 各电机最大限位 单位：° 度
    std::vector<int> LimitMin = {-100, -170, -20, -120, -100, -25, -60}; // 各电机最小限位 单位：° 度
    ...
    VectorXd Is_ChangeV;
    VectorXd DH_sign; // 电机摆放对DH建模正方向的影响,2电机贴纸朝里，4电机贴纸朝里
    ...
    // 各种标志位...
    double T_sample = 0.001; // 理想状态下指令发送到执行完毕的最大动作时间
    ...
    int need_p = 2;                 // 标志位，
    double rotate_resolution = 0.1; // 末端旋转速度的分辨率（越大速度越快）默认值
    double speed_resolution = 0.02; // 末端平移速度的分辨率（越大速度越快）默认值
...
{
    /*******************************ROS*****************************/
    ros::init(argc, argv, "r7_jacob_joy_left");
