...
public:
        ...
        else
        {
            ...
            if (left_or_right == 2 || left_or_right == 1)
            {
                if (endOrJointMode == JOINT_MODE) // 一般控制
                {
                    ...
                    for (int i = 0; i < N; i++)
                    {
                        firstCobraPubTosimMsg.firstSpeed[i] = ((1 + Is_ChangeV[i] * (cobra_Vel_level - 1)) * flag[i] * cobra_motor_sign[i]) * (2 * M_PI) / 180*3; // 仿真环境下需要修改单位为 弧度每秒
    ...
    void arm_param_init() // 变量初始化 *(a.m_storage.m_data)@9 在调试中查看矩阵或者向量中的值
    {
        ...
        LimitMax.resize(N);
        LimitMax << 180, 10, 30, -5, 160, 100, 105; // 各电机最大限位 单位：° 度N
        LimitMin.resize(N);
        LimitMin << -180, -160, -150, -120, -160, -18, -38; // 各电机最小限位 单位：° 度
...
{
    /*******************************ROS*****************************/
    ros::init(argc, argv, "r7_jacob_sim");
