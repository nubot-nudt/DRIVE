...
public:
    ...
    nubot_arm_node2()
    {
        ...
        secondCobraPubTosim_ = n.advertise<nubot_msgs::simSub>("/simSub_r", 1, true);

        secondCobraSubFromsim_ = n.subscribe("/simPub_r", 1, &nubot_arm_node2::secondCobraCallback, this);
        secondCobraPubTosimMsg.secondNeedMode = need_v;
    }
    ...
    void secondCobraCallback(const nubot_msgs::simPub &secondCobra)
    {

        for (int i = 0; i < N; i++) // 仿真使用
        {
            pos[i] = secondCobra.secondOriginalPos[i];
    ...
    void CartesianToJoint(bool mode)
    {
        ...
        MatrixXd Jacob = MatrixXd::Zero(6, N);
        for (int i = 0; i < N; i++)
        {
    ...
    void timeController(const ros::TimerEvent &e)
    {
        ...
        else
        {
            ...
            // 机械臂参数
            n.param<double>("kSpeedResolution", speed_resolution, 0.1);
            n.param<double>("kRotateResolution", rotate_resolution, 0.1);

            /*************算出机械臂各关节信息并输出到屏幕********/
            std::cout << "\033[47;31m右臂\033[0m各电机角度，单位：度以及底层电机控制量速度，单位：rpm:" << std::endl;
            for (int i = 0; i < N; i++)
            {
                std::cout << "电机[" << i + 1 << "] Angle:" << fixed << setprecision(2) << setiosflags(ios::left) << setw(7) << pos[i] << "[" << setw(4) << LimitMin[i] << "~" << setw(3) << LimitMax[i] << "];";
                std::cout << "速度控制指令: " << setiosflags(ios::left) << setw(6) << secondCobraPubTosimMsg.secondSpeed[i] << ";";
            ...
            if (left_or_right == 3 || left_or_right == 1)
            {
                if (endOrJointMode == JOINT_MODE) // 一般控制
                {
                    ...
                    if (flag_extend) // 一键展开  展开过程中不能松开按键 否则将停止展开   展开完毕需要松开“↓”和→
                    {
                        ...
                        {
                            // secondCobraPubTosimMsg.secondPos[5] = POS1[5]/ (360.0 / Motor_Reduction_Ratio[5]);
                            for (int i = 0; i < N; i++) // 将各个电机设置成位置模式
                            {
                                secondCobraPubTosimMsg.secondNeedMode = need_p;
                                secondCobraPubTosimMsg.secondPos[i] = pos_extend[i] / (360.0 / Motor_Reduction_Ratio[i]);
                                secondCobraPubTosimMsg.secondPosMode = 1;
                            }
                            flag_need_set_ModeP = 0;
                        }
                        secondCobraPubTosim_.publish(secondCobraPubTosimMsg);
                    ...
                    else if (flag_back) // 一键还原  还原过程中不能松开按键 否则将停止还原   还原完毕需要松开“↓”和→
                    {
                        ...
                        if (flag_need_set_ModeP) // 在if(flag_back)循环中，只执行一次这个子循环，即只设置一次位置模式，否则会出现：设置位置模式→控制电机位置→设置位置模式→控制电机位置→...一直循环这样会使电机不动。
                        {
                            ...
                            {
                                secondCobraPubTosimMsg.secondNeedMode = need_p;
                                secondCobraPubTosimMsg.secondPos[i] = pos_init[i] / (360.0 / Motor_Reduction_Ratio[i]);
                                secondCobraPubTosimMsg.secondPosMode = 1;
                            }
                            flag_need_set_ModeP = 0;
                        }
                    ...
                    else if (flagA) //  ps手柄的R2
                    {
                        ...
                        if (flag_need_set_ModeP) // 在if(flag_back)循环中，只执行一次这个子循环，即只设置一次位置模式，否则会出现：设置位置模式→控制电机位置→设置位置模式→控制电机位置→...一直循环这样会使电机不动。
                        {
                            ...
                            {
                                secondCobraPubTosimMsg.secondNeedMode = need_p;
                                secondCobraPubTosimMsg.secondPos[i] = pos[i] / (360.0 / Motor_Reduction_Ratio[i]);
                                secondCobraPubTosimMsg.secondPosMode = 1;
                                if (i == 3)
                                    secondCobraPubTosimMsg.secondPos[i] = -82 / (360.0 / Motor_Reduction_Ratio[i]);
                                if (i == 4)
                                    secondCobraPubTosimMsg.secondPos[i] = 0 / (360.0 / Motor_Reduction_Ratio[i]);
                                if (i == 5)
                                    secondCobraPubTosimMsg.secondPos[i] = 0 / (360.0 / Motor_Reduction_Ratio[i]);
                            }
                            flag_need_set_ModeP = 0;
                    ...
                    else if (flagY) // PS手柄的R1
                    {
                        ...
                        if (flag_need_set_ModeP) // 在if(flag_back)循环中，只执行一次这个子循环，即只设置一次位置模式，否则会出现：设置位置模式→控制电机位置→设置位置模式→控制电机位置→...一直循环这样会使电机不动。
                        {
                            for (int i = 0; i < N; i++) // 将各个电机设置成位置模式
                            {
                                ...
                                secondCobraPubTosimMsg.secondPosMode = 1;
                                if (i == 3)
                                    secondCobraPubTosimMsg.secondPos[i] = 87 / (360.0 / Motor_Reduction_Ratio[i]);
                                if (i == 4)
                                    secondCobraPubTosimMsg.secondPos[i] = 0 / (360.0 / Motor_Reduction_Ratio[i]);
                                if (i == 5)
                                    secondCobraPubTosimMsg.secondPos[i] = 0 / (360.0 / Motor_Reduction_Ratio[i]);
                    ...
                    for (int i = 0; i < N; i++)
                    {
                        secondCobraPubTosimMsg.secondSpeed[i] = ((1 + Is_ChangeV[i] * (cobra_Vel_level - 1)) * flag[i] * cobra_motor_sign[i]) * (2 * M_PI) / 180*3; // 仿真环境下需要修改单位为 弧度每秒
                        // secondCobraPubTosimMsg.secondSpeed[i] = (1 + Is_ChangeV[i] * (cobra_Vel_level - 1)) * cobra_Vel[i] * flag[i] * cobra_motor_sign[i]; //发布速度单位为....转每分钟
                    }
                    secondCobraPubTosim_.publish(secondCobraPubTosimMsg); // 发布速度
                }
                ...
                else if (endOrJointMode == END_MODE) // 末端控制
                {
                    ...
                    std::cout << std::endl;
                    std::cout << "dx:" << dx << ";dy:" << dy << ";dz:" << dz << ";rx:" << rx << ";ry:" << ry << ";rz:" << rz << ";" << std::endl;
                    std::cout << "相对于身体前方基坐标系坐标X:" << local_position(0) << ";Y:" << local_position(1) << ";Z:" << local_position(2) << ";" << std::endl;
                    for (int i = 0; i < N; i++)
                        secondCobraPubTosimMsg.secondSpeed[i] = DH_sign[i] * dtheta[i]; // 仿真环境下需要修改单位为弧度每秒
                    // secondCobraPubTosimMsg.secondSpeed[i] = DH_sign[i] * dtheta[i + 1] * Motor_Reduction_Ratio[i] * 60.0 / (3.1415926 * 2);
                    secondCobraPubTosim_.publish(secondCobraPubTosimMsg);
                }
            }
            if (endOrJointMode == JOINT_MODE)
    ...
    void arm_param_init() // 变量初始化 *(a.m_storage.m_data)@9 在调试中查看矩阵或者向量中的值
    {
        ...
        guodu_ << 1, 0, 0, 0, 0, cos(-M_PI / 4), -sin(-M_PI / 4), 0.2457, 0, sin(-M_PI / 4), cos(-M_PI / 4), 0, 0, 0, 0, 1; // 过渡矩阵为trans(y，d_)*rot（x，-45）d_=0.2457 为机器人身体坐标系与右肩关节坐标系在y轴的距离
        ...
        LimitMax.resize(N);
        LimitMax << 180, 160, 150, -5, 160, 18, 105; // 各电机最大限位 单位：° 度N
        LimitMin.resize(N);
        LimitMin << -180, -10, -30, -120, -160, -110, -38; // 各电机最小限位 单位：° 度
        temperature = VectorXd::Zero(N);                   // 电机的温度
        DianLiu = VectorXd::Zero(N);                       // 电机的电流

        secondSpeed = VectorXd::Zero(N);  // 速度指令
...
private:
    ...
    ros::Subscriber secondCobraSubFromsim_;
    ros::Publisher secondCobraPubTosim_;

    ros::Timer controller;

    nubot_msgs::simSub secondCobraPubTosimMsg;
    ...
    double speed_resolution = 0.2; // 末端平移速度的分辨率（越大速度越快）
...
{
    /*******************************ROS*****************************/
    ros::init(argc, argv, "r7_jacob_sim_2");
