...
public:
    ...
    void CartesianToJoint(bool mode)
    {
        ...
        ox = sin(theta[6]) * (cos(theta[5]) * (sin(theta[1]) * sin(theta[4]) + cos(theta[4]) * (cos(theta[1]) * cos(theta[2]) * sin(theta[3]) + cos(theta[1]) * cos(theta[3]) * sin(theta[2]))) - sin(theta[5]) * (cos(theta[1]) * sin(theta[2]) * sin(theta[3]) - cos(theta[1]) * cos(theta[2]) * cos(theta[3]))) - cos(theta[6]) * (cos(theta[4]) * sin(theta[1]) - sin(theta[4]) * (cos(theta[1]) * cos(theta[2]) * sin(theta[3]) + cos(theta[1]) * cos(theta[3]) * sin(theta[2])));

        oy = cos(theta[6]) * (cos(theta[1]) * cos(theta[4]) + sin(theta[4]) * (cos(theta[2]) * sin(theta[1]) * sin(theta[3]) + cos(theta[3]) * sin(theta[1]) * sin(theta[2]))) - sin(theta[6]) * (cos(theta[5]) * (cos(theta[1]) * sin(theta[4]) - cos(theta[4]) * (cos(theta[2]) * sin(theta[1]) * sin(theta[3]) + cos(theta[3]) * sin(theta[1]) * sin(theta[2]))) + sin(theta[5]) * (sin(theta[1]) * sin(theta[2]) * sin(theta[3]) - cos(theta[2]) * cos(theta[3]) * sin(theta[1])));

        oz = cos(theta[2] + theta[3]) * cos(theta[6]) * sin(theta[4]) - sin(theta[6]) * (sin(theta[2] + theta[3]) * sin(theta[5]) - cos(theta[2] + theta[3]) * cos(theta[4]) * cos(theta[5]));
        ...
        px = a2 * cos(theta[1]) + d4 * cos(theta[2] + theta[3]) * cos(theta[1]) - a4 * sin(theta[2] + theta[3]) * cos(theta[1]) - a3 * cos(theta[1]) * sin(theta[2]) + d6 * cos(theta[2] + theta[3]) * cos(theta[1]) * cos(theta[5]) - d6 * sin(theta[1]) * sin(theta[4]) * sin(theta[5]) - d6 * cos(theta[1]) * cos(theta[2]) * cos(theta[4]) * sin(theta[3]) * sin(theta[5]) - d6 * cos(theta[1]) * cos(theta[3]) * cos(theta[4]) * sin(theta[2]) * sin(theta[5]);

        py = a2 * sin(theta[1]) + d4 * cos(theta[2] + theta[3]) * sin(theta[1]) - a4 * sin(theta[2] + theta[3]) * sin(theta[1]) - a3 * sin(theta[1]) * sin(theta[2]) + d6 * cos(theta[2] + theta[3]) * cos(theta[5]) * sin(theta[1]) + d6 * cos(theta[1]) * sin(theta[4]) * sin(theta[5]) - d6 * cos(theta[2]) * cos(theta[4]) * sin(theta[1]) * sin(theta[3]) * sin(theta[5]) - d6 * cos(theta[3]) * cos(theta[4]) * sin(theta[1]) * sin(theta[2]) * sin(theta[5]);

        pz = d1 - a4 * cos(theta[2] + theta[3]) - d4 * sin(theta[2] + theta[3]) - a3 * cos(theta[2]) - (d6 * cos(theta[2] + theta[3]) * sin(theta[4] + theta[5])) / 2 - d6 * sin(theta[2] + theta[3]) * cos(theta[5]) + (d6 * sin(theta[4] - theta[5]) * cos(theta[2] + theta[3])) / 2;
        ...
        dtheta[2] = (a4 * dpy * sin(theta[2] + theta[3]) - d4 * dpy * cos(theta[2] + theta[3]) + d4 * d6 * day * cos(theta[2] + theta[3]) - a4 * d6 * day * sin(theta[2] + theta[3]) + pow(d4, 2) * dtheta[1] * pow(cos(theta[2] + theta[3]), 2) * cos(theta[1]) + pow(a4, 2) * dtheta[1] * pow(sin(theta[2] + theta[3]), 2) * cos(theta[1]) + a4 * dpz * cos(theta[2] + theta[3]) * sin(theta[1]) + d4 * dpz * sin(theta[2] + theta[3]) * sin(theta[1]) + a2 * d4 * dtheta[1] * cos(theta[2] + theta[3]) * cos(theta[1]) - a2 * a4 * dtheta[1] * sin(theta[2] + theta[3]) * cos(theta[1]) - a4 * d6 * daz * cos(theta[2] + theta[3]) * sin(theta[1]) - d4 * d6 * daz * sin(theta[2] + theta[3]) * sin(theta[1]) + a3 * a4 * dtheta[1] * sin(theta[2] + theta[3]) * cos(theta[1]) * sin(theta[2]) - 2 * a4 * d4 * dtheta[1] * cos(theta[2] + theta[3]) * sin(theta[2] + theta[3]) * cos(theta[1]) - a3 * d4 * dtheta[1] * cos(theta[2] + theta[3]) * cos(theta[1]) * sin(theta[2])) / (sin(theta[1]) * (a3 * d4 * cos(theta[2] + theta[3]) * cos(theta[2]) + a3 * a4 * cos(theta[2] + theta[3]) * sin(theta[2]) - a3 * a4 * sin(theta[2] + theta[3]) * cos(theta[2]) + a3 * d4 * sin(theta[2] + theta[3]) * sin(theta[2])));

        dtheta[3] = (d6 * daz - dpz + dtheta[2] * (a4 * sin(theta[2] + theta[3]) - d4 * cos(theta[2] + theta[3]) + a3 * sin(theta[2]))) / (d4 * cos(theta[2] + theta[3]) - a4 * sin(theta[2] + theta[3]));

        dtheta[5] = (dax * (cos(theta[1]) * sin(theta[2]) * sin(theta[3]) - cos(theta[1]) * cos(theta[2]) * cos(theta[3])) + day * (sin(theta[1]) * sin(theta[2]) * sin(theta[3]) - cos(theta[2]) * cos(theta[3]) * sin(theta[1])) + daz * (cos(theta[2]) * sin(theta[3]) + cos(theta[3]) * sin(theta[2])) - dtheta[1] * (ay * cos(theta[1]) * cos(theta[2]) * cos(theta[3]) - ax * cos(theta[2]) * cos(theta[3]) * sin(theta[1]) - ay * cos(theta[1]) * sin(theta[2]) * sin(theta[3]) + ax * sin(theta[1]) * sin(theta[2]) * sin(theta[3])) + dtheta[2] * (az * cos(theta[2]) * cos(theta[3]) - az * sin(theta[2]) * sin(theta[3]) + ax * cos(theta[1]) * cos(theta[2]) * sin(theta[3]) + ax * cos(theta[1]) * cos(theta[3]) * sin(theta[2]) + ay * cos(theta[2]) * sin(theta[1]) * sin(theta[3]) + ay * cos(theta[3]) * sin(theta[1]) * sin(theta[2])) + dtheta[3] * (az * cos(theta[2]) * cos(theta[3]) - az * sin(theta[2]) * sin(theta[3]) + ax * cos(theta[1]) * cos(theta[2]) * sin(theta[3]) + ax * cos(theta[1]) * cos(theta[3]) * sin(theta[2]) + ay * cos(theta[2]) * sin(theta[1]) * sin(theta[3]) + ay * cos(theta[3]) * sin(theta[1]) * sin(theta[2]))) / sin(theta[5]);
        ...
        dtheta[6] = (dtheta[1] * (ox * cos(theta[1]) + oy * sin(theta[1])) - dtheta[4] * (cos(theta[6]) * sin(theta[4]) + cos(theta[4]) * cos(theta[5]) * sin(theta[6])) - doy * cos(theta[1]) + dox * sin(theta[1]) + dtheta[5] * sin(theta[4]) * sin(theta[5]) * sin(theta[6])) / (cos(theta[4]) * sin(theta[6]) + cos(theta[5]) * cos(theta[6]) * sin(theta[4]));
    ...
    void auto_arm() // 自主模式下的机械臂运动控制，flag_auto==0则机械臂正在自主运动中，flag_auto==1为运动完成
    {
        ...
        if (flag_auto == 1)
        {
            ...
            const int SIZE = 4;
            double nums[SIZE];
            cout << "Enter " << SIZE << " float numbers range(-0.5~0.5): ";
            for (int i = 0; i < SIZE; i++)
            {
                cin >> nums[i];
            }
            end_roll = pos[5] + nums[3];
        ...
        else
        {
            ...
            if (abs(px - RHS(0, 3)) >= 0.01)
                dx = speed_resolution * end_mode_speed_level * -Sgn(px - RHS(0, 3));
            else
                dx = 0;
            if (abs(py - RHS(1, 3)) >= 0.01)
                dy = speed_resolution * end_mode_speed_level * -Sgn(py - RHS(1, 3));

            else
                dy = 0;
            if (abs(pz - RHS(2, 3)) >= 0.01)
                dz = speed_resolution * end_mode_speed_level * -Sgn(pz - RHS(2, 3));
            else
                dz = 0;
            cout << end_roll << endl;
            cout << pos[5] << endl;
            ...
            if (dx == 0 && dy == 0 && dz == 0)
            {
                if (abs(pos[5] - end_roll) >= 0.5)
                    dtheta[6] = rotate_resolution * end_mode_speed_level * -Sgn(pos[5] - end_roll);
                else
                {
                    dtheta[6] = 0;
                    flag_auto = 1; //.....旋转到位后需要夹持并旋转回来，方便下一次移动！！！
                    firstCobraPubTosimMsg.firstSpeed[6] = flag[6];
                    firstCobraPubTosim_.publish(firstCobraPubTosimMsg);
                }
            }
    ...
    void timeController(const ros::TimerEvent &e)
    {
        ...
        else
        {
            ...
            n.param<double>("firstCobra_a2", a2, 0.15);
            n.param<double>("firstCobra_a3", a3, -0.7);
            n.param<double>("firstCobra_a4", a4, -0.115);
            n.param<double>("firstCobra_d1", d1, 0.445);
            n.param<double>("firstCobra_d4", d4, 0.795);
            n.param<double>("firstCobra_d6", d6, 0.1);
            n.param<double>("firstCobra_a6", a6, 0.085);
            ...
            else // 默认为遥控模式
            {
                ...
                std::cout << "  a2:  " << a2 << "  a3:  " << a3 << "  a4:  " << a4 << "  a6:  " << a6 << std::endl;
...
private:
    ...
    **/
    ...
    double pos2theta[6] = {0, 0, 0, 0, 0, 0};                      // 当前位置到末端控制状态角转换差值 仿真环境
...
{
    /*******************************ROS*****************************/
    ros::init(argc, argv, "ibr2600_joy");
