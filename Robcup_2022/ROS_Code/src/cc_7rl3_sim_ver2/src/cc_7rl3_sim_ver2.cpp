...
public:
    ...
    void timeController(const ros::TimerEvent &e)
    {
        ...
        else
        {
            ...
            n.param<double>("firstCobra_a6", a6, 0.145);
            ...
            if (endOrJointMode == JOINT_MODE) // 一般控制
            {
                ...
                for (int i = 0; i < N1; i++)
                {
                    firstCobraPubTosimMsg.firstSpeed[i] = (((1 + Is_ChangeV[i] * (cobra_Vel_level - 1)) * flag[i] * cobra_motor_sign[i]) * (2 * 3.1415926) / 60) / 5; // 仿真环境下需要修改单位为 弧度每秒
...
{
    /*******************************ROS*****************************/
    ros::init(argc, argv, "cc_7rl3_sim_ver2");
