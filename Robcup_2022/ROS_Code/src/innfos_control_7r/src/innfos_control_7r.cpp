...
public:
    nubot_arm_node()
    {
        ...
        // initFTL = kw6ftsensorL->initCan(sensorL.debicIndex, 1000);
        /*********节点*********/
    ...
    void timeController(const ros::TimerEvent &e)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < N; i++)
            {
                ...
                {
                    ForceL[i] = *(FtL + i);
                    innfosPub.firstEndFS[i] = ForceL[i] * 9.81 - ForceL_drift[i];
                }
                FtR = kw6ftsensor->getFTdata(sensorR); // getFTdata使用的是同一块内存空间，指针指向后需要立即把数据输出出来
                for (int i = 0; i < 6; i++)
                {
                    ForceR[i] = *(FtR + i);
                    innfosPub.secondEndFS[i] = ForceR[i] * 9.81 - ForceR_drift[i];
                }
            ...
            innfos_pub_.publish(innfosPub);
            nodeCheck();
            // this_thread::sleep_for(std::chrono::microseconds(200)); // 加入CAN的帧间隔，防止数据获取失败
...
private:
    ...
    **/
    ...
    double ForceR[6] = {0};
    double ForceL[6] = {0};
    // double ForceR_drift[6] = {-1.3209, -0.5714, 0.4897, -0.0027, -0.1366, -0.0838};
    // double ForceL_drift[6] = {1.1642, 0.3521, 0.4575, 0.037, -0.0278, 0.0268};

    double ForceR_drift[6] = {0};
    double ForceL_drift[6] = {0};

    int Motor_PosV[15] = {100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100}; // 位置模式时，各底层电机转速,单位：每分钟多少转

    std::vector<int> LimitMax_left = {100, 5, 160, 5, 100, 110, 105};         // 各电机最大限位 单位：° 度
    std::vector<int> LimitMin_left = {-100, -170, -20, -120, -100, -25, -60}; // 各电机最小限位 单位：° 度
    std::vector<int> LimitMax_right = {100, 170, 20, 140, 100, 25, 60};       // 各电机最大限位 单位：° 度
    std::vector<int> LimitMin_right = {-100, -5, -160, -5, -100, -110, -105}; // 各电机最小限位 单位：° 度
