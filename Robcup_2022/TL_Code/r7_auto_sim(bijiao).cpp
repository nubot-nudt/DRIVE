...
public:
    ...
    nubot_arm_node2()
    {
        ...
        firstend_attitude_error_ = n.advertise<std_msgs::Float64MultiArray>("/end_attitude_error_l", 1, true);
    ...
    void cacul_RHS()
    {
        ...
        RHS = J__(0);
        local_attitude = RHS.topLeftCorner<3, 3>();
    ...
    void Target_control()
    {
        ...
        firstend_attitude_error_.publish(firstend_attitude_error);
    ...
    void end_force_control() // 末端阻抗控制+姿态闭环
    {
        ...
        double K_f[6] = {500, 200, 200, 200, 500, 500}; // 刚度参数
        ...
        if (pos_desire_save)
        {
            ...
            attitude_desire_new = local_attitude;
        ...
        else
        {
            ...
            // ↑↑↑↑↑↑↑↑↑↑↑attitude_err为每个时刻下期望姿态到保存姿态的偏差↑↑↑↑↑↑↑↑↑↑↑↑↑
            // ↓↓↓↓↓↓↓↓↓↓↓仅姿态控制用PID+前馈控制
            r_e_local = so3_ln_map(local_attitude.transpose() * attitude_desire_new); // 当前位置下的姿态误差，等价于end_pos_desire - local_position;
            // attitude_err = attitude_err * local_attitude.transpose() * attitude_desire;                                     // 姿态误差的累计值，用于PID+前馈控制
            attitude_desire_new = attitude_desire_new * r_e; // 更新每个时刻的期望姿态，等价于end_pos_desire的值
            // w_output = w_desire + 5 * r_e_local + 1 * SO3_daoshu(r_e_local) * w_desire + 0.0525 * so3_ln_map(attitude_err); // Ki=pow(Kp,2)/4,此时为临界阻尼，记Kp=1，Ki=0.25 ,其中积分项so3_ln_map(attitude_err)最好采用积分分离 ，在一开始跟踪误差较大时取消积分项目

            for (int i = 0; i < 3; i++)
                firstend_attitude_error.data[i] = r_e_local[i];
            // ↑↑↑↑↑↑↑↑↑↑↑仅姿态控制用PID+前馈控制
    ...
    void arm_param_init() // 变量初始化 *(a.m_storage.m_data)@9 在调试中查看矩阵或者向量中的值
    {
        ...
        attitude_desire_new.setIdentity();
        attitude_desire.setZero();
        attitude_err.setZero();

        firstend_pos_d.data.resize(3);
        firstend_pos_fb.data.resize(3);
        firstend_attitude_error.data.resize(3);
...
private:
    ...
    ros::Publisher firstend_attitude_error_;
    ros::Publisher firstend_pos_d_;
    ros::Publisher firstend_pos_fb_;

    ros::Timer controller;

    std_msgs::Float64MultiArray firstend_attitude_error;
