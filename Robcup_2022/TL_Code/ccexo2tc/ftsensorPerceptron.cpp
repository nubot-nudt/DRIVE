...
//printf("Fx= %2f Kg,Fy= %2f Kg,Fz= %2f Kg,Mx= %2f Kg/M,My= %2f Kg/M,Mz= %2f Kg/M\n",Force[0],Force[1],Force[2],Force[3],Force[4],Force[5]);
   ...
   double *ftsensorPerceptron::getFTdata(Sensor sensor)
   {
      ...
      //cout<< "成功接收数据" <<endl;
         ...
         {
            if (sensor.id > 25)
            sensorR[(sensor.id-26)/2].errornum=0;//通信畅通了错误数就清0
            else
            sensorL[(sensor.id-20)/2].errornum=0;
         ...
         if (sensor.id > 25)
         {
            ForceTorque[0]=ftRmsg[(sensor.id-26)/2].force.x/9.81;
            ForceTorque[1]=ftRmsg[(sensor.id-26)/2].force.y/9.81;
            ForceTorque[2]=ftRmsg[(sensor.id-26)/2].force.z/9.81;
            ForceTorque[3]=ftRmsg[(sensor.id-26)/2].torque.x/9.81;
            ForceTorque[4]=ftRmsg[(sensor.id-26)/2].torque.y/9.81;
            ForceTorque[5]=ftRmsg[(sensor.id-26)/2].torque.z/9.81;
            sensorR[(sensor.id-26)/2].errornum++;//错误标志位加1
            if (sensorR[(sensor.id-26)/2].errornum>3)
            {
               cout << "连续3次通讯中断" <<endl;
         ...
         {
            ForceTorque[0]=ftLmsg[(sensor.id-20)/2].force.x;
            ForceTorque[1]=ftLmsg[(sensor.id-20)/2].force.y;
            ForceTorque[2]=ftLmsg[(sensor.id-20)/2].force.z;
            ForceTorque[3]=ftLmsg[(sensor.id-20)/2].torque.x;
            ForceTorque[4]=ftLmsg[(sensor.id-20)/2].torque.y;
            ForceTorque[5]=ftLmsg[(sensor.id-20)/2].torque.z;
            sensorL[(sensor.id-20)/2].errornum++;//错误标志位加1
            if (sensorL[(sensor.id-20)/2].errornum>3)
            {
               cout << "连续3次通讯中断" <<endl;
               mode.data=11;//通讯终端进入stop模式
            }
