...
class Parent(object):
    ...
    def destroy(self):
        ...
        with self.update_child_actor_list_lock:
            for dummy_actor_id, actor in self.child_actors.items():
                actor.destroy()
                actor = None
    ...
    def get_dead_child_actors(self):
        ...
        :return:
        """
        for child_actor_id, child_actor in self.child_actors.items():
    ...
    def update_child_actors(self):
        ...
        if len(self.dead_child_actors) > 0 or len(self.new_child_actors) > 0:
            with self.update_child_actor_list_lock:
                ...
                for actor_id, actor in self.new_child_actors.items():
    ...
    def update(self):
        ...
        """
        with self.update_child_actor_list_lock:
            for actor_id, actor in self.child_actors.items():
                try:
                    actor.update()
